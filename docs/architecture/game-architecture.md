...
- **Getrennte Asset-Pipelines:** Web-Assets (2D-Sprites/Sounds) bleiben im bestehenden `public`/Komponenten-Workflow; 3D-Assets kommen aus `godot_assets/` (lokal, git-ignoriert, 331 MB) und werden erst nach Lizenzprüfung gezielt nach `/game/assets/` übernommen.
